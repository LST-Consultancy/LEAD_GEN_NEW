...
- LinkedIn posts (via Apify): a third-party scraper, not a LinkedIn API. Queries come from every named service, technology and alias crossed with buyer-intent expressions (looking for a consultant or partner, need help with, can anyone recommend, seeking a vendor or external team, RFP, hiring a freelancer or agency), shown and editable before running. Depth presets — Quick (4 queries × 1 page, ≤100 posts), Standard (8 × 2, ≤200), Deep (16 × 4, ≤500) — or custom limits, all capped by the workspace's "most posts one search may request" setting. Every post ends as qualified, needs review or rejected with one recorded reason; the funnel on Find Opportunities reconciles and states why the run stopped. Cancel stops between pages; Resume continues from the checkpoint without re-buying finished pages. Discovery never creates CRM leads.
