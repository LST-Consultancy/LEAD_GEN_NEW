...
export function OpportunitySearchView({ providers }: { providers: Awaited<ReturnType<typeof listOpportunityProviders>> }) {
  ...
  const queryPreviews = criteria ? providers.filter(p => (p.id === "brave" || p.id === "linkedin_posts") && selected.includes(p.id))
  ...
  return <section className="space-y-5 rounded-xl border border-border bg-surface p-6">
    ...
    {job && <SearchStatus job={job} providerName={id => providers.find(p => p.id === id)?.name ?? id} />}
    {results && results.total > 0 && <OpportunityTable data={results} />}
...
type ProviderOutcome = { status: string; found: number; message?: string; screened?: Record<string, number> };
...
function SearchStatus({ job, providerName }: { job: Search; providerName: (id: string) => string }) {
  ...
  return <div aria-live="polite" className="space-y-3">
    ...
    {outcomes.length > 0 && <ul className="space-y-1 text-sm">{outcomes.map(([id, o]) => <li key={id}><span className="font-medium">{providerName(id)}</span> <span className="text-secondary">· {PROVIDER_RESULT_LABEL[o.status] ?? o.status} · <span className="tabular-nums">{o.found}</span> {o.found === 1 ? "result" : "results"}</span>{screenedLine(o.screened) && <span className="block text-xs text-secondary tabular-nums">{screenedLine(o.screened)}</span>}{o.message && <span className="block text-xs text-secondary">{o.message}</span>}</li>)}</ul>}
