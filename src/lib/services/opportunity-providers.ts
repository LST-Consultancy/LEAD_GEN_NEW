...
export async function connectOpportunityProvider(ctx: AuthContext, provider: string, raw: unknown) {
  ...
  if (input.enabled && ["greenhouse", "lever", "ashby"].includes(provider) && !input.config.boards.length) throw new MutationError("Add at least one company job board.", "configuration_required", 422);
  if (input.enabled && provider === "adzuna" && (!input.config.appId || !input.config.countries.length)) throw new MutationError("Adzuna requires an application ID and at least one job market.", "configuration_required", 422);
  const existing = await db.providerConnection.findUnique({ where: { workspaceId_provider: { workspaceId: ctx.workspaceId, provider } } });
  if (descriptor.key && !input.apiKey && !existing?.encryptedCredentials) throw new MutationError("Supply the provider API key.", "credentials_required", 422);
