...
function csv(value: unknown) { let text = String(value ?? ""); if (/^[\s]*[=+\-@\t\r]/.test(text)) text = `'${text}`; return `"${text.replaceAll('"','""')}"`; }
export async function exportOpportunities(ctx: AuthContext, filters: unknown) {
  ...
  const rows: unknown[][] = [["Company","Domain","Opportunity","Opportunity Type","Intent Score","Fit Score","Posted Date","First Seen","Last Seen","Source","Source URL","Decision Maker","Title","Email","Email Status","Phone","Industry","Employees","Location","Technologies","Evidence Summary"]];
