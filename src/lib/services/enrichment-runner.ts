...
import { assessAuthor, mapEmployee, profileKeyOf, rankPeople, roleFocus, searchQueryFor, type PersonCandidate } from "@/lib/enrichment/people";
import { extractEmails, mapWebsiteItems, ownerOf, type FoundEmail } from "@/lib/enrichment/emails";
...
type Enrichment = { fields?: Record<string, FieldRecord>; stages?: Partial<Record<StageKey, { at: string; outcome: string }>> };
...
const profileValues = (p: CompanyProfile) => ({ domain: p.domain, website: p.website ? (safePublicHost(p.website) ? `https://${safePublicHost(p.website)}` : null) : null, linkedinUrl: p.linkedinUrl, description: p.description?.slice(0, 2000) ?? null, industry: p.industry, city: p.city, state: p.state, country: p.country, employeeCount: p.employeeCount, employeeBand: p.employeeBand });
...
async function savePerson(c: Ctx, companyId: string, p: PersonCandidate, source: string, extra: Record<string, unknown> = {}) {
  return db.$transaction(async tx => {
    ...
    if (!person) person = await tx.person.create({ data: { workspaceId: c.workspaceId, fullName: p.fullName, firstName: p.firstName, lastName: p.lastName, headline: p.headline, linkedinUrl: p.linkedinUrl, profileKey: p.profileKey, city: p.city, state: p.state, country: p.country ?? "Unknown" } });
    ...
    const evidence = { source, runId: c.runId, retrievedAt: new Date().toISOString(), association: { value: p.association, basis: p.associationBasis }, relevance: p.relevanceWhy, authority: p.authority, ...extra };
...
async function stagePeople(c: Ctx): Promise<StageOutcome> {
  ...
  const notes: string[] = [];
  const focus = roleFocus(c.opportunity.types);
  ...
  for (const source of c.opportunity.sources) {
    ...
    if (!a) continue;
    if (!a.ok) { notes.push(a.why); continue; }
    const { created } = await savePerson(c, company.id, { profileKey: profileKeyOf(a.profileUrl), linkedinUrl: a.profileUrl, fullName: a.name, firstName: null, lastName: null, headline: a.headline, title: /^(.*?)\s+(?:at|@)\s+/i.exec(a.headline)?.[1]?.trim() ?? "", city: null, state: null, country: null, association: "current", associationBasis: `${a.basis} Self-described; not otherwise confirmed.`, employer: company.name, relevance: 100, relevanceWhy: ["Wrote the opportunity's post."], authority: { inferred: true, seniority: null, likelyDecisionMaker: false, basis: "Post author; authority not assessed." }, emails: [], emailField: null }, "post_author", { postUrl: source.sourceUrl });
...
async function saveEmail(c: Ctx, companyId: string, f: FoundEmail, people: { id: string; fullName: string; firstName: string | null; lastName: string | null }[], counts: Record<string, number>, blocked: Set<string>, forcedPersonId?: string, extra: Record<string, unknown> = {}) {
  if (blocked.has(f.email)) { counts.suppressed++; return; }
  if (!f.sameDomain) { counts.setAside++; return; }
  const evidence = { ...f.evidence, retrievedAt: new Date().toISOString(), runId: c.runId, ...extra };
  const personId = forcedPersonId ?? (f.generic ? null : ownerOf(f.email, people));
  if (personId) {
    ...
    await db.contactMethod.create({ data: { workspaceId: c.workspaceId, personId, kind: "WORK_EMAIL", value: f.email, maskedValue: f.email.replace(/^(.).+@/, "$1***@"), isLocked: false, status: "UNVERIFIED", confidence: 50, source: `apify:${f.evidence.kind}`, verificationResult: "UNCHECKED", provenance: { discovery: evidence, allowedExport: c.conn.allowedExport } as Prisma.InputJsonValue } });
    counts.personal++;
    return;
  }
  ...
  await db.companyContactPoint.create({ data: { workspaceId: c.workspaceId, companyId, kind: "EMAIL", value: f.email, isGeneric: f.generic, source: `apify:${f.evidence.kind}`, evidence: { ...evidence, ...(f.generic ? {} : { note: "Looks like a person's address, but it does not match anyone found at this company." }) } as Prisma.InputJsonValue } });
  counts[f.generic ? "generic" : "unassigned"]++;
}

async function stageEmails(c: Ctx): Promise<StageOutcome> {
  const company = await db.company.findFirstOrThrow({ where: { id: c.opportunity.companyId, workspaceId: c.workspaceId } });
  const counts: Record<string, number> = { personal: 0, generic: 0, unassigned: 0, alreadyKnown: 0, setAside: 0, suppressed: 0, fromSources: 0, fromWebsite: 0, fromEmployeeSearch: 0 };
  ...
  for (const e of (c.result.employeeEmails as { personId: string; email: string; field: string | null }[] | undefined) ?? []) {
    found.push({ f: { email: e.email, generic: false, sameDomain: Boolean(domain && e.email.endsWith(`@${domain}`)), free: false, evidence: { kind: "employee_search", url: null, excerpt: null } }, personId: e.personId, extra: { field: e.field, note: "From the employee search's email mode; that output field is not in the Actor's documented example." } });
  ...
  for (const x of found) await saveEmail(c, company.id, x.f, people, counts, blocked, x.personId, x.extra);
  await stampStage(c.workspaceId, company.id, "emails", "searched");
  const added = counts.personal + counts.generic + counts.unassigned;
  const reason = [websiteNote, counts.setAside ? `${counts.setAside} addresses on other domains were set aside.` : null].filter(Boolean).join(" ") || undefined;
  if (!added && !counts.alreadyKnown) return { status: "no_matches", counts, reason: reason ?? "No business email address was found." };
...
async function stageVerify(c: Ctx): Promise<StageOutcome> {
  ...
  const points = await db.companyContactPoint.findMany({ where: { workspaceId: c.workspaceId, companyId, kind: "EMAIL" } });
...
const RUNNERS: Record<StageKey, (c: Ctx) => Promise<StageOutcome>> = { resolve: stageResolve, details: stageDetails, people: stagePeople, emails: stageEmails, verify: stageVerify, summary: stageSummary };
