...
export const providerConfigSchema = z.object({ boards: z.array(boardSchema).max(50).default([]), appId: z.string().max(200).default(""), countries: z.array(z.enum(["us", "gb", "in", "au", "at", "br", "ca", "de", "fr", "it", "nl", "nz", "pl", "sg", "za"])).max(15).default(["us", "gb", "in"]), maxQueries: z.number().int().min(1).max(6).default(3), pages: z.number().int().min(1).max(3).default(1), postsPerQuery: z.number().int().min(10).max(50).default(25), includeLinkedIn: z.boolean().default(true),
  // The most LinkedIn posts one search may request, whatever depth a member picks. Apify bills per post returned.
  maxPostsPerSearch: z.number().int().min(10).max(5000).default(500) });
...
export function discoveryProvider(workspaceId: string, id: string, rawConfig: z.input<typeof providerConfigSchema>, key?: string): OpportunitySourceProvider {
  ...
  async function search(query: SearchCriteria): Promise<SourceDocument[]> {
    ...
    try {
    if (id === "linkedin_posts") {
      const { searchLinkedInPosts } = await import("./linkedin-posts");
      return searchLinkedInPosts(workspaceId, config, query, key);
    }
    ...
    if (id === "brave") {
      ...
      // No site:linkedin.com/posts/ query: LinkedIn keeps posts out of search indexes, so it returned nothing.
      for (const term of webQueryTerms(query, config.maxQueries)) {
        const q = [term,...query.negativeKeywords.map(t => `-"${t.replaceAll('"', '')}"`), ...(query.domains.length ? [`(${query.domains.map(d => `site:${d}`).join(" OR ")})`] : [])].join(" ");
        ...
        for (const r of data.web?.results ?? []) {
          ...
          records.push({ provider: id, kind: (host === "linkedin.com" || host.endsWith(".linkedin.com")) && /^\/(posts|feed\/update)\//.test(new URL(sourceUrl).pathname) ? "LINKEDIN_PUBLIC_POST" : "PUBLIC_WEB", externalId: sourceUrl, sourceUrl, title: r.title, description: plainText(r.description), company: board ? { name: board.company, domain: board.domain, country: board.country } : { name: "" }, postedAt: null, status: "UNKNOWN", rawSourceReference: { searchQuery: q, sourcePageAge: r.page_age ?? null, resultTitle: r.title, resultSnippet: plainText(r.description), siteName: r.profile?.name ?? null } });
        }
      }
      return [...new Map(records.map(r => [r.externalId, r])).values()];
    }
  ...
  return { id, search, getOpportunity: async externalId => (await search({ services: [], technologies: [], opportunityTypes: [], locations: [], industries: [], employeeMin: null, employeeMax: null, dateRange: { days: 30 }, minimumIntent: 0, expandedTerms: [], negativeKeywords: [], domains: [] })).find(r => r.externalId === externalId) ?? null,
    ...
    healthCheck: async () => { if (id === "linkedin_posts") return (await import("./linkedin-posts")).checkLinkedInPostsAccess(workspaceId, key); if (!["brave", "greenhouse", "lever", "ashby", "adzuna"].includes(id)) return { ok: false, message: "LinkedIn capability unavailable for this connection." }; if (["greenhouse", "lever", "ashby"].includes(id) && !config.boards.length) return { ok: false, message: "Configure at least one company board/domain." }; await discoveryProvider(workspaceId, id, { ...config, boards: config.boards.slice(0, 1), countries: config.countries.slice(0, 1), maxQueries: 1, pages: 1, includeLinkedIn: false }, key).search({ services: [], technologies: [], opportunityTypes: [], locations: [], industries: [], employeeMin: null, employeeMax: null, dateRange: { days: 30 }, minimumIntent: 0, expandedTerms: ["implementation"], negativeKeywords: [], domains: [] }); return { ok: true, message: "Provider API responded successfully." }; },
    getCapabilities: () => ({ search: ["brave", "greenhouse", "lever", "ashby", "adzuna", "linkedin_posts"].includes(id), enrichment: false, verification: false, changes: false }) };
}
